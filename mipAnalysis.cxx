...
int main(int argc,char** argv)
{
  ...
  std::fstream outtex;
  outtex.open("/Users/arnaudsteen/cms/presentation/hgcal/tb_mip/10-2016/mip.tex",std::ios::out);
  std::fstream outfit;
  outfit.open("./txtFile/mipFitOutput.txt",std::ios::out);
  ...
  os.str("");
  os << "/data/root/hgcalSep2016/hgcalMuon.root";
  ...
  for( int il=0; il<nlayers; il++ ){
	...
	// hChi2.Fill( f.GetChisquare()/f.GetNDF() );
      ...
      cc.Update();
      os.str("");
      os << "~/cms/presentation/hgcal/tb_mip/10-2016/figs/Mip_Layer_" << il<< "_Skiroc" << is+1 << ".pdf";
      cc.SaveAs(os.str().c_str());

      outtex << "\\begin{frame}\n"
	     << "\t \\frametitle{Layer " << il << "; Skiroc " << is+1 << "}\n"
	     << "\t \\begin{figure}\n" 
	     << "\t \t \\includegraphics[width=1.0\\textwidth]{Mip_Layer_" << il<< "_Skiroc" << is+1 << ".pdf}\n"
	     << "\t \\end{figure}\n"
	     << "\\end{frame}"
	     << std::endl;
    }
